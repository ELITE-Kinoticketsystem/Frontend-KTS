<script lang="ts">
  ...
  $: {
    ...
    selectedSeats.sort((e, e2)=>{return e.type < e2.type ? 1 : 0;}).forEach((seat) => {
...
<div class="w-full h-full">
  <div class="flex flex-col w-full mx-auto sm:mt-1 lg:mt-2 mb-4">
    <p class="text-center font-semibold text-textWhite text-xs sm:text-sm  md:text-sm lg:text-lg xl:text-xl">
      Your selection
    </p>

    {#each distinctSeatsWithCount as seatWithCount}
      <div class="flex mx-auto">
        <p class="text-xs sm:text-sm md:text-sm lg:text-lg xl:text-xl text-white">
