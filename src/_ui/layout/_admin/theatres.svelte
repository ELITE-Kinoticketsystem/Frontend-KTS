<script lang="ts">
  ...
  import TheatreCreate from "../../templates/theatreCreate.svelte";
  ...
  function updateChart(index: number) {
    ...
    let tId = localChart.el.classList[0];
    let start = chartRange[index].start;
    let end = chartRange[index].end;
    fetchData(start, end, sortRanges.get(charFilterType[index]), tId).then(
      (data: any) => {
  ...
  $: onload = (el: any) => {
    ...
    fetchData(startDate, currentDate, sortRanges.get(filterTypes[0]), tId).then(
      (data: any) => {
        for (let i = 0; i < data.Date.length; i++) {
          data.Date[i] = new Date(data.Date[i]).toLocaleDateString();
        }
...
{#key reBuild}
  <div class="flex flex-col text-textWhite rounded-md shadow-md px-2 py-2">
    ...
    <div class="mt-10">
      <TheatreCreate on:create={() => reBuild++} />
    </div>
        ...
        >
          ...
          <div class="flex w-full h-full">
            <div class="w-full bg-headerBlue rounded-lg shadow p-4 md:p-6 mt-3">
              <div class="flex justify-between mb-5">
                <div class="grid gap-4 grid-cols-2">
                  <div>
                    ...
                    <p
                      class="text-darkTextWhite text-2xl leading-none font-bold"
                    >
                      {totalVisits[index]}
                    </p>
                  </div>
              ...
              <div id="{index}-chart" class={theatre.Name} use:onload></div>
            </div>
          </div>
        </div>
      {/each}
